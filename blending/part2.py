...
def blend(lapl_pyr_white, lapl_pyr_black, gauss_pyr_mask):
  ...
  for i in range(len(lapl_pyr_white)):
    black_mask = (1 - gauss_pyr_mask[i])
    blended =  gauss_pyr_mask[i] * lapl_pyr_white[i] + black_mask * lapl_pyr_black[i]
    blended_pyr.append(blended)
  return blended_pyr

def check_shapes(x, y):
  if y.shape != x.shape:
    print "ERROR shapes different: %s is not %s" % (( x.shape ), ( y.shape ))
...
def collapse(lapl_pyr):
  ...
  '''
  result = lapl_pyr[len(lapl_pyr) - 1]
  for i in reversed(range(len(lapl_pyr) - 1)):
    result = lapl_pyr[i] +  part1.lapl_expand(result, lapl_pyr[i].shape)
  return result
