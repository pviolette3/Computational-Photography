...
def lapl_pyramid(gauss_pyr):
  ...
  for k in range(len(gauss_pyr) - 1):
    output.append(lapl(k, gauss_pyr))
  output.append(gauss_pyr[len(gauss_pyr) - 1])
  return output

def lapl(k, gauss_pyr):
  '''get the kth laplacian of the pyramid'''
  orig_shape = gauss_pyr[k].shape
  next_expanded = lapl_expand(gauss_pyr[k+1], orig_shape)
  return gauss_pyr[k] - next_expanded
